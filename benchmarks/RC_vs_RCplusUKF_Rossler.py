...
def main():
    ...
    framework = RC_UKF(
        ...
        measurement_noise=np.eye(3) * 1e-10,
    ...
    for dim in range(3):
        axes[dim].plot(time, test_outputs[:, dim], label=f"Ground Truth", color='#53D8E5', linewidth=3.5)
        axes[dim].plot(time, rc_predicted[:, dim], label=f"Standard", linestyle='--', color='#6ECE63', linewidth=3.5)
        axes[dim].plot(time, rcukf_predicted[:, dim], label=f"RCUKF", linestyle='--', color='#9610DD', linewidth=3.5)
    ...
    # rc_errors = rc_predicted - test_outputs
    # rcukf_errors = rcukf_predicted - test_outputs
    ...
    # plt.suptitle("Rössler System: Prediction Errors (RC vs RCUKF)")
    # plt.tight_layout()
    # plt.show()
