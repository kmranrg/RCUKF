...
def main():
    ...
    framework = RC_UKF(
        ...
        measurement_noise=np.array([[measurement_noise_std**3]]),
    ...
    plt.plot(time, test_outputs[:, 0], label='Ground Truth', color='#53D8E5', linewidth=3.5)
    plt.plot(time, rc_predicted[:, 0], label='Standard RC', linestyle='--', color='#6ECE63', linewidth=3.5)
    plt.plot(time, rcukf_predicted[:, 0], label='RCUKF', linestyle='--', color='#9610DD', linewidth=3.5)
    ...
    # rc_errors = rc_predicted - test_outputs   # shape (N_test, 1)
    # rcukf_errors = rcukf_predicted - test_outputs   # shape (N_test, 1)

    # plt.figure(figsize=(9, 5))
    # plt.plot(time, rc_errors[:, 0], label="RC-Only Error", color="#6ECE63", linewidth=2.0)
    # plt.plot(time, rcukf_errors[:, 0], label="RCUKF Error", color="#9610DD", linewidth=2.0)

    # plt.title("Mackey-Glass: Prediction Errors (RC vs RCUKF)")
    # plt.xlabel("Time Step")
