...
def main():
    ...
    # -------------------------------------------------------
    # C) Plot Comparison: RC‑Only vs RCUKF vs Ground Truth
    # -------------------------------------------------------
    time   = np.arange(N_test)
    labels = ['x', 'y', 'z']
    ...
    for dim in range(3):
        axes[dim].plot(time, test_outputs[:, dim],
                    label=f"Ground Truth",   color='#53D8E5', linewidth=3.5)

        axes[dim].plot(time, rc_predicted_states[:, dim],
                    label=f"Standard RC", linestyle="--",
                    color='#6ECE63',  linewidth=3.5)

        axes[dim].plot(time, rcukf_predicted_states[:, dim],
                    label=f"RCUKF",  linestyle="--",
                    color='#9610DD',  linewidth=3.5)
        ...
        if dim == 0:
            axes[dim].legend(loc='best', fontsize=15)
    ...
    # # -------------------------------------------------------
    # # D) Plot Errors in a Separate Figure
    # # -------------------------------------------------------
    # # 1. Calculate errors for RC-only and RC+UKF
    # rc_errors = rc_predicted_states - test_outputs
    # rcukf_errors = rcukf_predicted_states - test_outputs

    # fig2, axes2 = plt.subplots(nrows=3, ncols=1, figsize=(9, 7), sharex=True)
    # for dim in range(3):
    #     axes2[dim].plot(
    #         time,
    #         rc_errors[:, dim],
    #         label=f"RC-Only Error in {labels[dim]}",
    #         color="#6ECE63",
    #         linewidth=2.0,
    #     )
    #     axes2[dim].plot(
    #         time,
    #         rcukf_errors[:, dim],
    #         label=f"RCUKF Error in {labels[dim]}",
    #         color="#9610DD",
    #         linewidth=2.0,
    #     )
    #     axes2[dim].set_ylabel(f"{labels[dim]} Error")
    #     axes2[dim].legend(loc="best")
    #     axes2[dim].grid(True)

    # axes2[-1].set_xlabel("Time Step")
    # plt.suptitle("Lorenz System: Prediction Errors (RC vs RCUKF)")
    # plt.tight_layout()
    # plt.show()
    ...
    #     axes2[dim].plot(time,
    #                     rc_errors[:, dim],
    #                     label=f"RC‑Only error",      # label kept but legend shown once
    #                     color="#6ECE63",
    #                     linewidth=2.0)
